...
- **Tags do Git (ao vivo)**: em vez de digitar as tags, o North roda `git fetch --tags` no caminho do repositório informado (pela mesma conexão SSH) e monta o dropdown com as tags reais do servidor no momento de executar — sem risco de digitar a tag errada. Se o remote for HTTPS e pedir autenticação, o North usa **Usuário Git** e **Senha Git** da seção Secrets da conexão (os mesmos secrets dos passos de deploy). Se o repositório só for legível/gravável como outro usuário (ex: pasta de deploy dona de `www-data`), o North também usa a **Senha sudo** da conexão, quando cadastrada, pra rodar o fetch com `sudo`. Sem os secrets necessários preenchidos, o fetch falha e a mensagem pede pra configurar Secrets.
