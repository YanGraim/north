...
- grade em **Resultados** com fonte 13px: clique no **nome** do cabeçalho seleciona a coluna inteira (Shift / ⌘/Ctrl para intervalo ou soma/tira); arraste nas **células** (cima/baixo ou um retângulo) para só alguns valores; o **ícone** cicla a ordenação; arraste o cabeçalho para reordenar. O clique na tabela carrega na hora (**100 linhas** por página) e busca mais ao rolar (rodapé mostra `100+`, depois o total exato no fim). Query livre continua com teto de 1000 linhas
...
- em qualquer result set (tabela, query livre, JOIN ou view), duplo clique, **F2** ou **Enter** abre o editor da célula; clique simples continua selecionando a linha. Inserir / Duplicar / Excluir (botão direito) só na aba de **tabela** que não é view
- **Salvar** / **Cancelar** aparecem no rodapé assim que o valor muda (⌘/Ctrl+S salva). Salvar executa `UPDATE` de verdade: na aba de tabela, na própria relação; na query livre, na primeira tabela do `FROM` (com JOIN, não nas tabelas do JOIN). É preciso ter chave primária no result set; `DISTINCT` / `GROUP BY` / `UNION` / `WITH` não gravam (o resultado não mapeia 1:1). Auto-commit da sessão continua valendo
