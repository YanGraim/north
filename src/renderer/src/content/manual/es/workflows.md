...
- **Tags de Git (en vivo)**: en vez de escribir las tags, North ejecuta `git fetch --tags` en la ruta del repositorio indicada (por la misma conexión SSH) y arma el dropdown con las tags reales del servidor en el momento de ejecutar — sin riesgo de escribir la tag equivocada. Si el remote es HTTPS y pide autenticación, North usa **Usuario Git** y **Contraseña Git** de la sección Secretos de la conexión (los mismos secretos que usan los pasos de deploy). Si el repositorio solo es legible/escribible como otro usuario (ej: una carpeta de deploy propiedad de `www-data`), North también usa la **Contraseña sudo** de la conexión, cuando está configurada, para ejecutar el fetch con `sudo`. Sin los secretos necesarios completos, el fetch falla y el mensaje pide configurar Secretos.
