...
- grid en **Resultados** a 13px: clic en el **nombre** del encabezado selecciona la columna entera (Shift / ⌘/Ctrl para intervalo o alternar); arrastra las **celdas** (arriba/abajo o un rectángulo) para solo algunos valores; el **icono** cicla la ordenación; arrastra el encabezado para reordenar; arrastra el **borde derecho** del encabezado para cambiar el ancho (doble clic en el borde ajusta al contenido visible). El clic en la tabla carga al instante (**100 filas** por página) y pide más al hacer scroll (el pie muestra `100+`, luego el total exacto al llegar al final). La pestaña Query pagina igual los `SELECT`/`WITH` sin `LIMIT`/`TOP`/`FETCH`; el SQL original permanece en el editor (Guardar no usa el `OFFSET`)
